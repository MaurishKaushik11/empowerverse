#!/usr/bin/env python3
"""
Sample Data Generator for EmpowerVerse (PostgreSQL Version)
Creates realistic sample data for presentation and demo purposes using PostgreSQL
"""
...
def create_recommendation_logs(users, posts):
    ...
    for user in users:
        ...
        for _ in range(random.randint(3, 5)):
            # Recommend 3-min(8, len(posts)) posts
            max_recommendations = min(len(posts), 8)
            num_recommendations = random.randint(3, max_recommendations)
            recommended_posts = random.sample([p.id for p in posts], num_recommendations)
...
def main():
    ...
    print("🚀 Creating sample data for EmpowerVerse (PostgreSQL)...")
    ...
    try:
        ...
        print("\n🚀 Your EmpowerVerse PostgreSQL database is now ready for presentation!")
